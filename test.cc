...
TEST(OperatorBrac, False) {
  ...
  EXPECT_THROW(matrix_a(-1, -1), std::out_of_range);
}
...
TEST(Inverse, True) {
  ...
  EXPECT_THROW(matrix_k.InverseMatrix(), std::logic_error);
}
TEST(Get, True) {
  S21Matrix matrix_a(3, 3);
...
TEST(Set, True) {
  ...
  EXPECT_THROW(S21Matrix check2(-1, -2), std::out_of_range);
  S21Matrix matrix_b(3, 3);
  ...
  EXPECT_THROW(matrix_b.SetRows(-1), std::out_of_range);
